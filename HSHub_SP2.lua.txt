...
function Signature.AttachToWindow(Window, opts)
    ...
    -- ── HUB ──
    local sHub = tab:CreateSection("HUB")
    sHub:AddInfo("Name", Signature.Brand)

    -- ── CREATOR ──
    local sCreator = tab:CreateSection("CREATOR")
    sCreator:AddInfo("Created by", Signature.Creator)

    -- ── DISCORD ──
    local sDiscord = tab:CreateSection("DISCORD")
    sDiscord:AddLabel(Signature.DiscordShort, Color3.fromRGB(60, 200, 230))
    sDiscord:AddButton({
...
function LinoriaCompat.new(HSHub, opts)
    ...
    function library:CreateWindow(winopts)
        ...
        function windowWrap:AddTab(name, icon)
            ...
            function tabWrap:AddLeftGroupbox(title)  return _wrapGroup(title) end
            function tabWrap:AddRightGroupbox(title) return _wrapGroup(title) end
...
-- ═══════════════════════════════════════════════════════════════════
--   HSHub Credits tab + ready notification
-- ═══════════════════════════════════════════════════════════════════
pcall(function()
    Sig.AttachToWindow(library._hsWindow)
end)
pcall(function()
    HSHub:Notify('HS Hub loaded · Specter 2 · HS-SP2-V2', 'ok', 3)
end)
